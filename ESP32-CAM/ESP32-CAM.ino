...
const char* serverUrl = "http://example.com/upload_image";
...
void loop() {
  ...
  // Check if conditions are met to send the image
  if (isConditionMet(fb)) {
...
bool isConditionMet(camera_fb_t *fb) {
  // Example condition: Send image if the image size is greater than 50KB
  return (fb->len > 50 * 1024);
}
